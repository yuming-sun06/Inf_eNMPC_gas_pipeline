...
cost_labels = [title.replace("\n", "\n") for title, *_ in runs]
total_costs = [
    compute_cost_kwh(pwr, pwr_cols, dt=3600, power_to_kw=POWER_SCALE)
    for _, _, _, _, pwr, _ in runs
]
fig_cost, ax_cost = plt.subplots(
    figsize=(max(8, 1.8 * len(runs)), 5.5),
    constrained_layout=True,
)
bars = ax_cost.bar(
    np.arange(len(runs)),
    total_costs,
    color=[cmap10(i % 10) for i in range(len(runs))],
)
ax_cost.set_xticks(np.arange(len(runs)), cost_labels)
ax_cost.set_ylabel("Total cost (kWh)")
ax_cost.set_title(f"Total compressor cost over 0–{X_MAX_HOURS} h")
ax_cost.tick_params(axis="x", labelsize=8)
ax_cost.bar_label(bars, labels=[f"{cost:.2f}" for cost in total_costs],
                  padding=3, fontsize=9)
cost_min = min(total_costs)
cost_max = max(total_costs)
cost_span = cost_max - cost_min
ax_cost.set_ylim(cost_min, cost_max + 0.15 * (cost_span or 1.0))
