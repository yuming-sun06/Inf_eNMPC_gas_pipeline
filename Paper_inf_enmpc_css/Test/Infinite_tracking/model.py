...
def TIME_sets(m, Opt, infinite_horizon=False):
    if infinite_horizon:
        # Transformed infinite-time coordinate tau = tanh(gamma*(t-t_bar)).
...
def STATIONS_vars(m):
    ...
    m.compressor_beta = pyo.Var(
        m.Stations, m.Times, bounds=(1.05, 2.0),
        within=pyo.NonNegativeReals, initialize=1.05)
...
def PIPE_vars(m, scale, networkData, infinite_horizon=False):
    ...
    if infinite_horizon:
        m.drhodt = DerivativeVar(m.pipe_rho, wrt=m.Times)
        m.gamma = pyo.Param(initialize=1.0, mutable=True)
...
def PIPE_mass_constr(
    ...
    if infinite_horizon:
        def PIPE_mass_balance_infhorizon_rule(m, p, vol, t):
            if t == m.Times.first():
            ...
            return (m.gamma * (1 - t**2) * m.drhodt[p, vol + 1, t]
                    * V / (N - 1)
                    == w * scale['w'] - w_next * scale['w']
                    - cons * scale['w'])

        m.PIPE_mass_balance = pyo.Constraint(
            m.Pipes_VolCenterC, m.Times,
            rule=PIPE_mass_balance_infhorizon_rule)
        return m
    ...
    if method == 'COLLOCATION' and dynamic:
        ...
        def PIPE_mass_balance_coll_rule(m, p, vol, t):
            ...
            # m.Times is indexed in hours, so drho_dt is d(rho)/d(hour) and the
            # LHS is kg/h, while the mass flows are kg/s.  The 3600 converts the
            # RHS to kg/h -- same convention as the infinite block's collocation
            # mass balance in infinite_block.py.
            return (m.drho_dt[p, vol + 1, t] * V_cell
                    == 3600.0 * (w * scale['w'] - w_next * scale['w']
                                 - cons * scale['w']))
...
def buildNonLinearModel(
    ...
    ''' Sets '''
    # Time
    m = TIME_sets(m, Opt, infinite_horizon=infinite_horizon)
    # Network
    ...
    if (not infinite_horizon
            and Opt.get('finite_diff_time') == 'COLLOCATION'
            and Opt.get('dynamic', False)):
