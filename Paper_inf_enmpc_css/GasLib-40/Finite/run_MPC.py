...
from stability import (
    ...
    update_tracking_cost_plant_prev,
)
...
def make_controller_and_plant(
    ...
    if not warmup_periodic:
        ...
        # variables.  Load them separately for Lyapunov tracking and the MPC
        # terminal condition, without adding a periodic constraint here.
...
def cycle_lyapunov_values(m_controller, cycle_length):
    """Split the controller V into cycle 1 and cycles 2--10."""
    terminal_time = m_controller.Times.last()

    def tracking_sum(times):
        return (
            sum(
    ...
    first_cycle_times = [
        t for t in m_controller.Times
        if t != terminal_time and float(t) < float(cycle_length)
    ]
    remaining_cycle_times = [
        t for t in m_controller.Times
        if t != terminal_time and float(t) >= float(cycle_length)
    ]
    v_1_cycle = pyo.value(tracking_sum(first_cycle_times))
    v_9_cycle = pyo.value(tracking_sum(remaining_cycle_times))
    return v_1_cycle, v_9_cycle, v_1_cycle + v_9_cycle
...
def run_nmpc(
    ...
    for iteration in range(simulation_steps):
        ...
        v_1_cycle, v_9_cycle, v_total = cycle_lyapunov_values(
            m_controller, cycle_length)
        lyapunov_values[iteration] = {
            "V_1_cyle": v_1_cycle,
            "V_9_cycle": v_9_cycle,
            "V_total": v_total,
        }
        ...
        print(
            f"V_1_cyle={v_1_cycle:.6e}, "
            f"V_9_cycle={v_9_cycle:.6e}, "
            f"V_total={v_total:.6e}",
            flush=True,
        )
        ...
        m_controller.lyapunov_function_prev.set_value(
            pyo.value(m_controller.lyapunov_function_current)
        )
        plant_tf = m_plant.Times.last()
        reference_t = sample_time
        update_tracking_cost_plant_prev(
            m_controller,
            m_plant,
            plant_tf,
    ...
    print(timer)
    return m_plant, m_controller, sim_data, lyapunov_values, iteration_times
...
def write_results(
        sim_data, m_plant, lyapunov_values, iteration_times,
    ...
    lyapunov_df = pd.DataFrame.from_dict(
        lyapunov_values, orient="index")
    lyapunov_df.index.name = "iteration"
    lyapunov_df = lyapunov_df[
        ["V_1_cyle", "V_9_cycle", "V_total"]]
    with pd.ExcelWriter(
            output_path,
            engine="openpyxl",
            mode="a",
            if_sheet_exists="replace") as writer:
        lyapunov_df.to_excel(writer, sheet_name="controller_lyapunov")
...
def main(
    ...
    m_plant, _, sim_data, lyapunov_values, iteration_times = run_nmpc(
    ...
    output_path = os.path.join(_BASE_DIR, "finite-10-cycle.xlsx")
    write_results(
        sim_data,
        m_plant,
        lyapunov_values,
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(_BASE_DIR, "finite-10-cycle.log")
    tee = Tee(log_path)
    try:
        print("soft =", soft)
