...
class ControlGasNet:
    def __init__(self):
        ...
        self.NFE_infinite_horizon = 10
        self.NCP_infinite_horizon = 4
        self.Gamma = 0.1

        # Lyapunov
        self.Stability_constraint = True
        self.delta = 0.1
        ...
        # The demand period and the available CSS reference period need not be
        # identical.  css.xlsx contains the initial point again as its final
        # (cycle-closing) row, hence the subtraction by one.
        self.CSS_cycle_length = (
            len(pd.read_excel(self.ocss_data_path, sheet_name="interm_p")) - 1
        )
    ...
    def make_controller_and_plant_model_finite(self):
        original_css_loader = controller_plant_builder.set_initial_state_from_css

        def _set_initial_state(model, css_file_path):
            original_css_loader(model, css_file_path)
        ...
        controller_plant_builder.set_initial_state_from_css = _set_initial_state
        try:
            ...
            controller_plant_builder.set_initial_state_from_css = original_css_loader
    ...
    def _tail_css_index(self, tau):
        t_bar = self.Horizon
        gamma = pyo.value(self.m_controller_inf.gamma)
        t = t_bar + 1.0/gamma*np.arctanh(tau)
        return int(round(t % self.CSS_cycle_length)) % self.CSS_cycle_length
    ...
    def make_actual_controller_model(self):
        ...
        # p and P are hard terminal equalities; rho has no terminal constraint.
        self.m_controller.infinite.terminal_cost = 0.0
        
        for s in self.m_controller.infinite.sink_node_set:
            self.m_controller.infinite.wCons[s,:,1].fix(pyo.value(self.m_controller.finite.wCons[s, 0, self.Horizon]))

    # ============================================================================
    # Lyapunov tracking and stability
    # ============================================================================
    def make_tracking_expressions(self):
        m = self.m_controller
        fin = m.finite
        inf = m.infinite
        # ---- finite-horizon parts (t = 0 .. T-1, one hour apart) -----------
        m.V_p_finite = sum(
            (fin.interm_p[p, vol, t] - fin.interm_p_ocss[p, vol, t])**2
            for p, vol in fin.Pipes_VolExtrR_interm
            for t in fin.Times if t != fin.Times.last())

        m.V_beta_finite = sum(
            (fin.compressor_beta[s, t] - fin.compressor_beta_ocss[s, t])**2
            for s in fin.Stations
            for t in fin.Times if t != fin.Times.last())

        # ---- infinite-horizon (tail) parts ---------------------------------
        # Use the same Radau weights and omit only the singular point tau=1.
        gamma = inf.gamma
        m.V_p_infinite = sum(
        ...
        m.V_beta_infinite = sum(
            (inf.compressor_beta[s, tau]
             - fin.compressor_beta_ocss[
    ...
    def make_lyapunov_function(self):
        m = self.m_controller

        m.lyapunov_function = (m.V_p_finite
                               + m.V_p_infinite
                               + m.V_beta_finite
                               + m.V_beta_infinite)

        # ---- descent inequality --------------------------------------------
        m.lyapunov_prev = pyo.Param(initialize=0.0, mutable=True)
        m.tracking_cost_plant_prev = pyo.Param(initialize=0.0, mutable=True)
        m.delta = pyo.Param(initialize=self.delta, mutable=True)
        def _stability_constraint(m):
            return (m.lyapunov_function
                    <= m.lyapunov_prev
                    - m.delta*m.tracking_cost_plant_prev)
        m.stability_constraint = pyo.Constraint(rule=_stability_constraint)
        m.stability_constraint.deactivate()

    def plant_tracking_cost(self):
        plant = self.m_plant
        fin = self.m_controller.finite
        tf = plant.Times.last()
        p_term = sum((pyo.value(plant.interm_p[p, vol, tf])
                      - pyo.value(fin.interm_p_ocss[p, vol, tf]))**2
                     for p, vol in fin.Pipes_VolExtrR_interm)
        beta_term = sum((pyo.value(plant.compressor_beta[s, tf])
                         - pyo.value(fin.compressor_beta_ocss[s, tf]))**2
                        for s in fin.Stations)
        return p_term + beta_term
    ...
    def run_nmpc(self):
        ...
        # Create the finite and tail pressure/beta tracking expressions.
        self.make_tracking_expressions()

        # Add the Lyapunov function and descent constraint when stability is enabled.
        if self.Stability_constraint:
            self.make_lyapunov_function()
        ...
        for step, i in enumerate(
            ...
            V_finite = (pyo.value(self.m_controller.V_p_finite)
                        + pyo.value(self.m_controller.V_beta_finite))
            V_infinite = (pyo.value(self.m_controller.V_p_infinite)
                          + pyo.value(self.m_controller.V_beta_infinite))
            V_total = V_finite + V_infinite
            # Create the current-step entry before the plant solve appends
            # ell_plant.  Without this initialization, accessing
            # lyapunov_terms[t_base] raises KeyError at MPC step 0.
            if self.Stability_constraint:
                self.lyapunov_terms[t_base] = {
                    "V_finite": V_finite,
                    "V_infinite": V_infinite,
                    "V_total": V_total,
                }
            ...
            print(f"  V: finite={V_finite:.6e}, infinite={V_infinite:.6e}, total={V_total:.6e}", flush=True)
            ...
            if self.Stability_constraint:
                ell_plant = self.plant_tracking_cost()
                self.lyapunov_terms[t_base]['ell_plant'] = ell_plant
            ...
            if self.Stability_constraint:
                self.m_controller.lyapunov_prev.set_value(
                    self.lyapunov_terms[t_base]["V_total"])
                self.m_controller.tracking_cost_plant_prev.set_value(
                    ell_plant)
                if not self.m_controller.stability_constraint.active:
                    self.m_controller.stability_constraint.activate()
    ...
    def save_results(self):
        ...
        lyapunov_terms = getattr(self, "lyapunov_terms", None)
        if lyapunov_terms:
            lyapunov_df = pd.DataFrame.from_dict(
                lyapunov_terms, orient="index")
            lyapunov_df = lyapunov_df.reindex(sorted(lyapunov_df.index))
            lyapunov_df.index.name = "t [h]"
            lyapunov_df["dV"] = lyapunov_df["V_total"].diff()
            columns = [
                "V_finite", "V_infinite", "V_total", "dV", "ell_plant"
            ]
            lyapunov_df = lyapunov_df[
                [column for column in columns
                 if column in lyapunov_df.columns]
            ]
            with pd.ExcelWriter(
                    self.results_path,
                    engine="openpyxl",
                    mode="a",
                    if_sheet_exists="replace") as writer:
                lyapunov_df.to_excel(writer, sheet_name="lyapunov")
...
if __name__ == "__main__":
    RESULTS_PATH = str(BASE_PATH / "Inf.xlsx")
    LOG_PATH = str(BASE_PATH / "Inf.txt")
