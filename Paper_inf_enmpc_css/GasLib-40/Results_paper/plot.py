...
COST_RUNS = [
    ("finite-1-cycle_tracking.xlsx", "1-cycle tracking"),
    ("Inf_tracking.xlsx", "Inf/10-cycle tracking"),
    ("finite-1-cycle.xlsx", "1-cycle eNMPC"),
    ("finite-10-cycle.xlsx", "10-cycle eNMPC"),
    ("Inf.xlsx", "Inf eNMPC"),
]
...
OUTPUT_COST_24H_PNG = "cost_24h.png"
...
# Cost plots use five equally spaced bars in the requested order.
cost_runs = []
...
COST_POSITIONS = np.arange(len(cost_runs))
...
ax_cost.set_xticks(COST_POSITIONS, cost_labels)
...
def plot_horizon_cost_comparison(end_hour, out_file):
    ...
    ax_h.set_xticks(COST_POSITIONS, cost_labels)
    ax_h.set_ylabel("Energy (kWh)")
    ax_h.yaxis.set_major_formatter(StrMethodFormatter("{x:,.0f}"))
...
plot_horizon_cost_comparison(24, OUTPUT_COST_24H_PNG)
...
def write_results(path):
    ...
    for fn, title in TIME_FILES:
        ...
        cost_24h = compute_cost_through_hour(
            pwr, pwr_cols, end_hour=24, dt=3600,
            power_to_kw=POWER_SCALE,
        )
        ...
        cost_24h_str = (
            f"{cost_24h:.2f} kWh" if cost_24h is not None else "N/A"
        )
        cost_60h_str = (
        ...
        lines.append(
            f"    0-24 h cost (including t=24): {cost_24h_str}")
        lines.append(
