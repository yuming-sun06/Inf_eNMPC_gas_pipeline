...
def set_tracking_objective(
        ...
        coeff_P,
        coeff_pSource3):
    """Replace the economic objective with the complete tracking objective."""
    if m_controller.component("tracking_p") is None:
        m_controller.tracking_p = pyo.Expression(
            expr=coeff_p*sum(
                (m_controller.interm_p[p, vol, t]
                 - m_controller.interm_p_ocss[p, vol, t])**2
                for p, vol in m_controller.Pipes_VolExtrR_interm
                for t in m_controller.Times
                if t != m_controller.Times.last()
            )
        )
        m_controller.tracking_P = pyo.Expression(
            expr=coeff_P*sum(
                (m_controller.compressor_P[s, t]
                 - m_controller.compressor_P_ocss[s, t])**2
                for s in m_controller.Stations
                for t in m_controller.Times
                if t != m_controller.Times.last()
            )
        )
        m_controller.tracking_pSource3 = pyo.Expression(
            expr=coeff_pSource3*sum(
                (m_controller.pSource["source_3", t]
                 - m_controller.pSource_ocss["source_3", t])**2
                for t in m_controller.Times
                if t != m_controller.Times.last()
            )
        )
    ...
    tracking_objective = (
        m_controller.tracking_p
        + m_controller.tracking_P
        + m_controller.tracking_pSource3
    )
    if terminal:
        tracking_objective += terminal_penalty_coeff*sum(
            m_controller.terminal_p_slack_abs[p, vol]
            for p, vol in m_controller.Pipes_VolExtrR_interm
        )

    m_controller.ObjFun.deactivate()
    if m_controller.component("obj") is not None:
        m_controller.del_component("obj")
    m_controller.obj = pyo.Objective(expr=tracking_objective)
    return m_controller
...
def make_controller_and_plant(
    ...
    _, m_controller = run_model(
        ...
        periodic_constraints=warmup_periodic and terminal,
        soft=terminal,
        tracking=True,
        coeff_p=coeff_p,
        coeff_P=coeff_P,
    ...
    if not (warmup_periodic and terminal):
    ...
    # For a non-periodic terminal setup, the terminal components are created
    # here (after run_model), so rebuild the already-tracking objective once to
    # include its soft terminal penalty.  Otherwise the third run_model solve
    # has already used the complete tracking objective.
    ...
    if terminal and not warmup_periodic:
        # These terminal components were deliberately added after the three
        # non-periodic warm-up solves, so solve their tracking problem once
        # before using the trajectory to initialize the plant.
        warmup_solver = pyo.SolverFactory("ipopt")
        warmup_solver.options["tol"] = 1e-4
        warmup_solver.options["linear_solver"] = "ma57"
        print("\n========== tracking warm-up with terminal penalty ==========")
        print("degrees_of_freedom =", degrees_of_freedom(m_controller))
        warmup_result = warmup_solver.solve(m_controller, tee=True)
        pyo.assert_optimal_termination(warmup_result)
...
def run_nmpc(
    ...
    for iteration in range(simulation_steps):
        ...
        print(
            ...
            f"pSource3={pyo.value(m_controller.tracking_pSource3):.6e}, "
            f"terminal={terminal_value:.6e}, total={objective_value:.6e}",
            flush=True,
        )
        ...
        m_controller.pSource_ocss["source_3", controller_tf].fix(
            m_controller.pSource_ocss["source_3", controller_t0]
        )
...
def main(
    ...
    output_path = os.path.join(_BASE_DIR, "finite-10-cycle_tracking_e-2.xlsx")
    write_results(
        sim_data,
        m_plant,
        iteration_times,
...
if __name__ == "__main__":
    terminal = False
    warmup_periodic = False
    terminal_penalty_coeff = 1e6
    coeff_p = 1e-1
    coeff_P = 1e-1
    coeff_pSource3 = 1e-1
    log_path = os.path.join(_BASE_DIR, "finite-10-cycle_tracking_e-1.log")
    tee = Tee(log_path)
    try:
        ...
        print("coeff_P =", coeff_P)
        print("coeff_pSource3 =", coeff_pSource3)
        main(
            terminal=terminal,
            warmup_periodic=warmup_periodic,
            terminal_penalty_coeff=terminal_penalty_coeff,
