...
def dynamic_demand_calculation(
        ...
        epsilon=0.0):
    """Return the 5% sinusoidal demand used by this GasLib-40 case.

    This case's CSS and transformed infinite tail use a 1/20 amplitude.  The
    ...
    for idx in m.wCons:
        if idx[0].startswith(('sink', 'exit')):
            ...
            profile = nominal + (nominal + epsilon)*shape/20.0
            if extended_profile:
...
def set_initial_state_from_css(m, css_file_path):
    """Load a complete t=0 guess and fix the model's initial states."""
    sheet_components = {
