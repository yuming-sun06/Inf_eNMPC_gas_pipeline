...
class ControlGasNet:
    def __init__(self):
        ...
        # Tracking coefficients.  The same coefficient is used in the finite
        # horizon and infinite tail for each tracked variable.
        self.coeff_p = 1.0
        self.coeff_P = 1.0
        self.terminal = True
    ...
    def make_actual_controller_model(self):
        ...
        if self.terminal:
            ...
            inf.terminal_p_slack_neg = pyo.Var(
                inf.Pipes_VolExtrR_interm,
                initialize=0, domain=pyo.NonNegativeReals)
            inf.terminal_power_slack_pos = pyo.Var(
                inf.Stations, initialize=0, domain=pyo.NonNegativeReals)
            inf.terminal_power_slack_neg = pyo.Var(
                inf.Stations, initialize=0, domain=pyo.NonNegativeReals)
            inf.terminal_rho_slack_pos = pyo.Var(
                inf.Pipes_VolExtrR_interm,
                initialize=0, domain=pyo.NonNegativeReals)
            inf.terminal_rho_slack_neg = pyo.Var(
                inf.Pipes_VolExtrR_interm,
                initialize=0, domain=pyo.NonNegativeReals)
            ...
            inf.terminal_rho = pyo.Constraint(
                inf.Pipes_VolExtrR_interm, rule=_terminal_rho)

            inf.terminal_cost = (
                sum(inf.terminal_power_slack_pos[s]
                    + inf.terminal_power_slack_neg[s]
                ...
                + sum(inf.terminal_rho_slack_pos[p, vol]
                      + inf.terminal_rho_slack_neg[p, vol]
                      for p, vol in inf.Pipes_VolExtrR_interm))
    ...
    # ============================================================================
    # Pressure and compressor-power tracking
    # ============================================================================
    ...
    def run_nmpc(self):
        ...
        # Create finite- and infinite-horizon pressure/P tracking terms.
...
if __name__ == "__main__":
    RESULTS_PATH = str(BASE_PATH / "Inf_tracking.xlsx")
    LOG_PATH = str(BASE_PATH / "Inf_tracking.txt")
    TERMINAL = False
